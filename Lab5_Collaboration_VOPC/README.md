...
between objects, with hierarchical message numbering — and a **VOPC (View of
Participating Classes)** diagram showing the analysis classes with their
...
Messages 3.1–3.5 are the validation sub-messages sent by
...
| `RegistrationValidator` | control | Prerequisites, capacity, time conflict, course load (BR6–BR9). |
...
4.5.1 is the conflict checker's query back to `aFaculty`, which enforces BR3 and
BR4.
...
controllers with their Thymeleaf views — the layers set out in the
[architecture document](../Lab3_Architecture/eRegistrar_Architecture.md) and
realised in the Lab 7 eRegistrar application.
