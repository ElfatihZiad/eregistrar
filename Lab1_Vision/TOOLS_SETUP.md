...
Technology stack chosen for the eRegistrar project: **Java + Spring Boot**, with
PlantUML for UML modelling and Git/GitHub for version control.
...
| Apache Maven | Java build tool | Supplied per-project by the Maven Wrapper (`mvnw`) generated with each Spring Boot project in Lab 7 — no global install required |

## UML tool

PlantUML was chosen over a GUI tool (StarUML, Visual Paradigm, draw.io) because
the diagrams are plain text: they live in the Git repository next to the
documents, diff meaningfully between commits, and re-render identically on any
machine. Every diagram in Labs 2–5 has its `.puml` source checked in beside the
rendered `.png`.
...
These require an installer/administrator action, so they are listed here rather
than performed automatically. Everything in Labs 6 and 7 in this repository
builds and runs on the JDK 11 that is already installed.
